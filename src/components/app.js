      ...
      gapi.client
        ...
        .then(function() {
          return gapi.client.request({
            path: `https://www.googleapis.com/calendar/v3/calendars/${CALENDAR_ID}/events?maxResults=11&orderBy=updated&timeMin=${moment().toISOString()}&timeMax=${moment().add(10, 'days')
              .toISOString()}`
          });
        })
        .then(
      ...
      return (
        ...
        >
          ...
          <span className="badge">
            {moment(event.start.dateTime).format("h:mm a")}{" "} to {moment(event.end.dateTime).format("h:mm a")},{" "}
            {moment(event.end.dateTime).diff(
              moment(event.start.dateTime),
              "minutes"
            )}{" "}
            minutes, {moment(event.start.dateTime).format("DD-MM-YYYY")}{" "}
          </span>
        </a>
      );
    });
